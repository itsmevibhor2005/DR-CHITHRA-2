...
const SECTIONS = [
  ...
  { name: "Reads", key: "reads" },
];

const API_BASE = `${import.meta.env.VITE_BACKEND_URL}/api/watch`;

export default function WatchOutFor() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`${API_BASE}/${selectedTab.key}/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Watch Out For</h1>
        ...
        {(data[selectedTab.key] || []).map((item) => (
          ...
          >
            <div>
              ...
              <a href={item.link} className="text-blue-600" target="_blank">
                {item.link}
              </a>
            </div>
